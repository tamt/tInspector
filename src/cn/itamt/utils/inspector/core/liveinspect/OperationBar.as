package cn.itamt.utils.inspector.core.liveinspect
{
	...
	public class OperationBar extends Sprite
	{
		...
		private var _style : Style = new Style(0, 1, 0x393939);
		...
		public function init() : void
		{
			_inited = true;
			// 按钮
			_btns = [_moveBtn = new InspectorViewMoveButton, _pareBtn = new InspectorViewParentButton, _childBtn = new InspectorViewChildButton, _prevBtn = new InspectorViewPrevButton, _broBtn = new InspectorViewBrotherButton];
			_infoBtn = new PropertiesViewButton;
			if (showPropBtn) _btns.push(_infoBtn);
			_struBtn = new StructureViewButton;
			if (showStructBtn) _btns.push(_struBtn);
			_filterBtn = new FilterManagerButton;
			if (showFilterBtn) _btns.push(_filterBtn);
			_3dBtn = new Inspector3DButton();
			if (show3DBtn) _btns.push(_3dBtn);
			_closeBtn = new InspectorViewCloseButton();
			if (showCloseBtn) _btns.push(_closeBtn);
			this.relayout();
		...
		private function relayout() : void
		{
			...
			graphics.drawRoundRectComplex(0, 0, btn.x + btn.width + 10, _height, 8, 8, 0, 8);
			graphics.endFill();
		}
		...
		public function set showCloseBtn(value : Boolean) : void
		{
			...
			if (_inited)
			{
				var i : int = _btns.indexOf(_closeBtn);
				if (_showCloseBtn)
				{
					if (i < 0) _btns.splice(_btns.length - 1, 0, _closeBtn);
				}
				else
				{
					if (i >= 0)
