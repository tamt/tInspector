package cn.itamt.utils.inspector.plugins.deval
{
	...
	public class DEval extends Object
	{
		...
		{
			if (_this && !(_this is Stage))
			{
				// D.eval内部直接访问对象的prototype属性，会导致运行出错。这个用于修正这个错误。
				try
				{
					// if(_this["constructor"] && _this["constructor"].prototype && _this["constructor"].prototype.prototype){
					_this["constructor"].prototype.prototype = null;
				}
				catch(e : Error)
				{
					var errMsg : String = e.message.toString();
					if (errMsgList[errMsg]) errMsg = errMsgList[errMsg];
					// get message from array if its exist
			...
			code = code.replace(/trace/g, 'printf');
			// we need always to return values
			try
			{
				if (_this is Stage)
				{
					D.eval(code, null, null);
				}
				else
				{
