package cn.itamt.utils.inspector.plugins.deval
{
	...
	public class DEvalPanel extends InspectorViewPanel
	{
		...
		public function DEvalPanel(w : Number = 300, h : Number = 300, autoDisposeWhenRemove : Boolean = true, resizeable : Boolean = true, closeable : Boolean = true)
		{
			...
			_output = InspectorTextField.create(InspectorLanguageManager.getStr("DEvalRunTip"), 0xffffff, 12);
			...
			var helpItem : ContextMenuItem = new ContextMenuItem(InspectorLanguageManager.getStr("DEvalRef"));
			cm.customItems.push(clearItem);
			cm.customItems.push(helpItem);
			clearItem.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT, onMenuItemSelect);
			helpItem.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT, onMenuItemSelect);
		...
		private function onMenuItemSelect(event : ContextMenuEvent) : void
		{
			...
			{
				case InspectorLanguageManager.getStr("DEvalRef"):
					navigateToURL(new URLRequest("http://www.riaone.com/products/deval/"), "_blank");
					break;
				case InspectorLanguageManager.getStr("Clear"):
					_output.text = "";
					break;
				default:
			}
