	...
	public class TransformTool extends Sprite {
		...
		public function set target(d : DisplayObject) : void {
			...
			if (!d) {
				...
				if (d == _target || d == this || contains(d) || (d is DisplayObjectContainer && (d as DisplayObjectContainer).contains(this))) {
					return;
				}
		...
		private function updateMatrix(useMatrix : Matrix = null, counterTransform : Boolean = true) : void {
			if (_target) {
				...
				if(useMatrix == null) {
					if(_target.transform.concatenatedMatrix == null) {
						if(_target.transform["matrix3D"] == null) {
						} else {
							var pMx : Matrix = _target.parent.transform.concatenatedMatrix.clone();
							pMx.concat(_target.transform.matrix);
							_toolMatrix = pMx;
						} 
					} else {
						_toolMatrix = DisplayObjectTool.getConcatenatedMatrix(_target);	
					}	
				} else {
					_toolMatrix = useMatrix.clone();
				}
				if (counterTransform) {
		...
		public function apply() : void {
			if (_target) {
				...
				if (_target.parent) {
					var invertMatrix : Matrix = DisplayObjectTool.getConcatenatedMatrix(target.parent);
					invertMatrix.invert();
