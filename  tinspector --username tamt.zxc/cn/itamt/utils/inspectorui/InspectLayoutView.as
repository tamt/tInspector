package cn.itamt.utils.inspectorui {
	 ...
	 * @author tamt
	 */
	public class InspectLayoutView extends Sprite {
		...
		public function InspectLayoutView() {
			super();
		...
		private function onRollOver(evt : MouseEvent) : void {
			if(evt.target is DisplayObjectLayoutSymbol) {
				...
				if(!(dols.symbolTo is Stage)) {
					...
					dispatchEvent(new Event(InspectLayoutView.LIVE_INSPECT));
				}
			}
		}
		...
		private function onClick(evt : MouseEvent) : void {
			if(evt.target is DisplayObjectLayoutSymbol) {
				if(!((evt.target as DisplayObjectLayoutSymbol).symbolTo is Stage)) {
					...
					_target = (evt.target as DisplayObjectLayoutSymbol).symbolTo;
					dispatchEvent(new Event(InspectLayoutView.INSPECT));
				}
			}
		}
